...
def import_responsibles(request): 
    ...
    for i in range(3, 63):
        ...
        # club responsible
        try: 
            club_responsible = ClubResponsible.objects.get(club_id=club.id, user_id=user.id)
        except ObjectDoesNotExist:
            function = sh['B' + str(i)].value
            club_responsible = ClubResponsible(club_id=club.id, user_id=user.id, function=function)
            club_responsible.save()

    return HttpResponse("Done import responsibles from responsables.xlsx")
...
def process_teams(request):
    ...
    for team in teams:
        team_name = team.name.split()[1]
        club = Club.objects.filter(name__icontains=team_name).first()
        if club: 
            team.club_id = club.id
            team.save()
    return HttpResponse("Done adding club to teams")

def process_users(request): 
    '''
    try to add club_id by analysing club_responsible table
    '''
    users = User.objects.filter(club_id__isnull=True)
    for user in users:
        club_responsible = ClubResponsible.objects.filter(user_id = user.id).first()
        if club_responsible: 
            user.club_id = club_responsible.club_id
            user.save()
    return HttpResponse("Done")

def process_clubs(request):
    '''
    add slug
    '''
    clubs = Club.objects.filter(slug__isnull=True)
    for club in clubs:
        club.slug = slugify(club.name)
        club.save()
    return HttpResponse('Done adding slug for clubs')